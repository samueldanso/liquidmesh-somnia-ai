...
import { WRAPPED_STT_ABI, MOCK_USDC_ABI, LIQUIDITY_VAULT_ABI } from '@/lib/abis'
...
export function DepositForm({}: DepositFormProps) {
	...
	const { writeContract: writeApproveWSTT, data: approveWSTTHash } = useWriteContract()
	const { writeContract: writeApproveUSDC, data: approveUSDCHash } = useWriteContract()
	...
	useEffect(() => {
		if (latestTx && lastTxRef.current !== latestTx) {
			...
			toast.success(`Agent executed: ${latestTx.slice(0, 10)}...${latestTx.slice(-8)}`, {
